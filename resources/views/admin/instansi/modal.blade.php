    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        ...
        <div class="p-4 overflow-y-auto max-h-[calc(100vh-180px)]">
            <form action="{{ route('data-instansi.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="website_link" class="inline-block mb-2 text-base font-medium">Link Website<span
                            class="text-red-500">*</span></label>
    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        ...
        <div class="max-h-[calc(100vh-180px)] p-4 overflow-y-auto">
            <form action="{{ route('data-instansi.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="sejarah" class="inline-block mb-2 text-base font-medium">Sejarah</label>
                    <textarea name="sejarah" id="sejarahEdit" rows="6" placeholder="Sejarah Instansi"
                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">{{ old('sejarah') }}</textarea>
                </div>
...
@push('js')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ClassicEditor
                .create(document.querySelector('#sejarahEdit'), {
                    toolbar: ['undo', 'redo', '|', 'heading', '|', 'bold', 'italic', 'link', 'bulletedList',
                        'numberedList',
                        'blockQuote', '|', 'fontfamily', 'fontsize', 'fontColor', '|', 'outdent', 'indent',
                    ],
